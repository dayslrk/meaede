...
class CompressionNet:
    ...
    def extract_error(self,x,x_dash):
        e=self.loss(x,x_dash)
        return e
    def inference(self, x):
        ...
        with tf.variable_scope("CompNet"):
            # AutoEncoder
            z_c,r = self.compress(x)
            x_dash,t = self.reverse(z_c)
            # compose feature vector
            z = self.extract_feature(x, x_dash, z_c)
            t=t[::-1]
            e1=self.extract_error(r[0],t[0])
            e2=self.extract_error(r[1],t[1])
            e3=self.extract_error(r[2],t[2])
        ...
        return tf.concat([z,e1,e2,e3], axis=1), x_dash
